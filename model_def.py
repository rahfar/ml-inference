"""Vessel track predictor – shared between training and serving."""
import torch
...
class VesselTrackPredictor(nn.Module):
    ...
    def __init__(self, hidden_size: int = 256, num_layers: int = 2):
        ...
        self.encoder = nn.LSTM(
            HISTORY_FEATURES, hidden_size, num_layers,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.decoder(h[-1])                      # (batch, 30)
        return out.view(-1, FUTURE_STEPS, FUTURE_FEATURES)  # (batch, 15, 2)
