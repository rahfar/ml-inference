...
_gRPC matches Flask throughput (req/s) but uses only **37% of the bandwidth** — binary encoding pays off on both serialization CPU and network._
...
_Memory for native run measured via `ps aux` RSS sum (gunicorn master + worker); no Docker involved._
